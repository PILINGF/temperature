...
def main():
  ...
  with tf.Session() as sess:
    ...
    for i in range(MAX_STEP):
      ...
      _, _predict, _loss = sess.run([train_op, predict_op, loss_op], feed_dict={x: train_data, y: actual_data, rate: 0.05})
      if i % 100 == 0:
        print('step: %05d loss: %0.05f' % (i, _loss))
      if i % 1000 == 0:
        test_data, test_target = data.test_data()
        _loss, _predict = sess.run([loss_op, predict_op], feed_dict={x: test_data, y: test_target, rate: 0.1})
